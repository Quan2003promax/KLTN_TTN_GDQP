...
begin
  ...
  for Col := 0 to Grid.Columns.Count - 1 do
    Worksheet.Cells[4, Col + 1] := Grid.Columns[Col].Title.Caption;
  for Row := 1 to Grid.DataSource.DataSet.RecordCount do
  begin
    Grid.DataSource.DataSet.RecNo := Row;
    for Col := 0 to Grid.Columns.Count - 1 do
      Worksheet.Cells[Row + 4, Col + 1] := Grid.Columns[Col].Field.AsString;
  end;
  ...
  try
    ...
    begin
      case td.ModalResult of
        100: // Mở file
          begin
            FilePath := ExtractFilePath(ParamStr(0)) + 'QDcapchungchi.pdf';

            if not FileExists(FilePath) then
            begin
              ResStream := TResourceStream.Create(HInstance, 'QDCAPCHUNGCHI', 'PDF');
              try
                ResStream.SaveToFile(FilePath);
              finally
                ResStream.Free;
        ...
        101: // Lưu về máy
          begin
            ...
            try
              ...
              SaveDlg.Filter := 'PDF Files (*.pdf)|*.pdf';
              SaveDlg.DefaultExt := 'pdf';
              SaveDlg.FileName := 'QDcapchungchi.pdf';
...
begin
  ...
  // Truy vấn tổng hợp điểm từ SQL Server
  ClientDataSet2.Active := False;
  ClientDataSet2.CommandText :=
    'WITH MaxScores AS ( ' +
    '  SELECT B.MaSV, B.MaHP, MAX(B.DiemMax) AS DiemMax, MAX(CAST(B.DaNop AS INT)) AS DaNop, HP.TinChi ' +
    '  FROM BangDiem B ' +
    ...
    // Tín chỉ tích lũy
    '  SUM(CASE ' +
    '    WHEN LEFT(H.MaSV,2) = ''19'' AND M.DiemMax > 3.9 AND M.DaNop = 1 AND M.MaHP IN (''QP211006'',''QP211007'',''QP211008'',''QP211009'') THEN M.TinChi ' +
    '    WHEN LEFT(H.MaSV,2) IN (''20'',''21'',''22'') AND M.DiemMax > 5 AND M.DaNop = 1 AND M.MaHP IN (''QP211006'',''QP211011'',''QP211012'',''QP211013'') THEN M.TinChi ' +
    '    ELSE 0 END) AS [Tín chỉ tích lũy], ' +

    // Trung bình cộng
    '  CAST(SUM(CASE ' +
    '    WHEN LEFT(H.MaSV,2) = ''19'' AND M.MaHP IN (''QP211006'',''QP211007'',''QP211008'',''QP211009'') THEN M.DiemMax * M.TinChi ' +
    '    WHEN LEFT(H.MaSV,2) IN (''20'',''21'',''22'') AND M.MaHP IN (''QP211006'',''QP211011'',''QP211012'',''QP211013'') THEN M.DiemMax * M.TinChi ' +
    ...
    '      WHEN LEFT(H.MaSV,2) = ''19'' AND M.MaHP IN (''QP211006'',''QP211007'',''QP211008'',''QP211009'') THEN M.DiemMax * M.TinChi ' +
    '      WHEN LEFT(H.MaSV,2) IN (''20'',''21'',''22'') AND M.MaHP IN (''QP211006'',''QP211011'',''QP211012'',''QP211013'') THEN M.DiemMax * M.TinChi ' +
    '      ELSE 0 END) AS FLOAT) / 8 >= 5 ' +
    '    AND SUM(CASE ' +
    '      WHEN LEFT(H.MaSV,2) = ''19'' AND M.DiemMax > 3.9 AND M.DaNop = 1 AND M.MaHP IN (''QP211006'',''QP211007'',''QP211008'',''QP211009'') THEN M.TinChi ' +
    '      WHEN LEFT(H.MaSV,2) IN (''20'',''21'',''22'') AND M.DiemMax > 5 AND M.DaNop = 1 AND M.MaHP IN (''QP211006'',''QP211011'',''QP211012'',''QP211013'') THEN M.TinChi ' +
    '      ELSE 0 END) = 8 THEN ''PASS'' ' +
    '    ELSE ''FAIL'' END AS [Xếp loại], ' +

    // Điểm từng học phần
    '  MAX(CASE WHEN M.MaHP = ''QP211006'' THEN M.DiemMax ELSE NULL END) AS [QP211006], ' +
    '  MAX(CASE WHEN M.MaHP = ''QP211007'' THEN M.DiemMax ELSE NULL END) AS [QP211007], ' +
    '  MAX(CASE WHEN M.MaHP = ''QP211008'' THEN M.DiemMax ELSE NULL END) AS [QP211008], ' +
    '  MAX(CASE WHEN M.MaHP = ''QP211009'' THEN M.DiemMax ELSE NULL END) AS [QP211009], ' +
    '  MAX(CASE WHEN M.MaHP = ''QP211011'' THEN M.DiemMax ELSE NULL END) AS [QP211011], ' +
    '  MAX(CASE WHEN M.MaHP = ''QP211012'' THEN M.DiemMax ELSE NULL END) AS [QP211012], ' +
    '  MAX(CASE WHEN M.MaHP = ''QP211013'' THEN M.DiemMax ELSE NULL END) AS [QP211013] ' +
    'FROM MaxScores M ' +
    'JOIN HSSV H ON M.MaSV = H.MaSV ' +
    'WHERE H.MaLop = :MaLop ' +
  ...
  // Đổi tiêu đề các cột học phần theo TenHP
  ...
  // Ẩn cột không liên quan theo khóa
  MaSV := ClientDataSet2.FieldByName('MaSV').AsString;
  for i := 0 to DBGrid2.Columns.Count - 1 do
  begin
...
begin
  ...
  DBGrid10.Canvas.Brush.Color := RGB(255, 255, 204); // Màu vàng nhạt
  DBGrid10.DefaultDrawColumnCell(Rect, DataCol, Column, State);
end;
