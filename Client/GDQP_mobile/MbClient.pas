...
type
  TForm1 = class(TForm)
    ...
    Label3: TLabel;
    procedure LoadClassByTeacher(const MaKhoa, MaLop: string);
procedure LoadClassByStudent(const MaSV: string);
...
procedure TForm1.LoadTeacherInfo(const AMaGV: string);
begin
  try
    ClientDataSet1.Close;
    ClientDataSet1.CommandText :=
      'SELECT IDCB, holot, ten FROM CBSoyeu WHERE IDCB = ''' +AMaGV + '''';
    ClientDataSet1.Open;
...
begin
  if Label3.Text = 'Giảng viên' then
    ShowMessage('Đây là giảng viên. Hiển thị chọn Khoa + Lớp.')
  else
    LoadClassByStudent(FMaSV);
end;
...
procedure TForm1.LoadClassByStudent(const MaSV: string);
begin
  try
    ClientDataSet1.Close;
    ClientDataSet1.CommandText :=
      'SELECT H.MaSV, H.HoLot, H.Ten ' +
      'FROM HSSV H ' +
      'WHERE H.MaLop = (SELECT MaLop FROM HSSV WHERE MaSV = ''' + MaSV + ''')';
    ClientDataSet1.Open;
  except
    on E: Exception do
      ShowMessage('Lỗi khi tải danh sách lớp: ' + E.Message);
  end;
end;
 procedure TForm1.LoadClassByTeacher(const MaKhoa, MaLop: string);
begin
  try
    ClientDataSet1.Close;
    ClientDataSet1.CommandText :=
      'SELECT MaSV, HoLot, Ten FROM HSSV ' +
      'WHERE MaKhoa = ''' + MaKhoa + ''' AND MaLop = ''' + MaLop + '''';
    ClientDataSet1.Open;
  except
    on E: Exception do
      ShowMessage('Lỗi khi tải danh sách lớp: ' + E.Message);
